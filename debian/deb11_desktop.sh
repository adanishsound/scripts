...
# deb11_desktop.sh
#
# Live install script for deb + ZFS 
#
...
# For now in bullseye RT kernels are not compatible with ZFS modules
...
# Suffix for setup of /dev/disk/by-id devices. Unset if running in a vm (/dev/vda).
PART_SUFFIX="-part"
...
update_shadow_hash() {
    sed -i -e 's/pam_unix.so obscure sha512$/pam_unix.so obscure sha512 rounds=999999/' /etc/pam.d/common-password
}
...
init_packages() {
    ...
    echo "installing tasksel standard packages ..."
...
setup_sources() {
    { \
        ...
        echo -e "deb http://${DEB_SEC_REPO} ${DEB_NAME}-security  main contrib non-free"; \
...
common_desktop_packages() {
    ...
    (
        cd /usr/share/themes
        ${CURL_CMD} https://github.com/EliverLara/Nordic/releases/download/2.0.0/Nordic.tar.xz | tar xvJf -
    )
}
...
# Scratch space for building large source projects
# and dev artifacts caches.
# Fast and loose ...
...
chroot_install_post() {
    ...
    # update pwd hash rounds before pwds are set...
    update_shadow_hash
...
setup_chroot_common() {
    ...
    # use openntpd instead of the systemd ntp
...
usage() {
    ...
    echo ""
    echo " Use Debian live : user/live"
    echo "   $ sudo -s "
    echo ""
